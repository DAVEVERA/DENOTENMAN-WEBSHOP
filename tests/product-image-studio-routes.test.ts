...
import { GET as listTrash } from "../app/api/admin/products/[id]/images/trash/route";
import { POST as restoreTrash } from "../app/api/admin/products/[id]/images/trash/[trashId]/restore/route";
...
const trashContext = { params: Promise.resolve({ id: "product_1", trashId: "trash_1" }) };
...
test("every product image API rejects unauthenticated requests before processing input", async () => {
  const responses = await Promise.all([
    ...
    listTrash(new NextRequest("http://localhost/api/admin/products/product_1/images/trash"), productContext),
    restoreTrash(new NextRequest("http://localhost/api/admin/products/product_1/images/trash/trash_1/restore", { method: "POST" }), trashContext),
  ]);

  assert.deepEqual(responses.map((response) => response.status), [401, 401, 401, 401, 401, 401, 401, 401]);
