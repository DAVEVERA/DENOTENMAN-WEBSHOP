...
const cartActionCopy = {
  nl: { added: "Toegevoegd!", goToCart: "Naar winkelwagen", continueShopping: "Verder winkelen" },
  en: { added: "Added!", goToCart: "Go to cart", continueShopping: "Continue shopping" },
  fr: { added: "Ajouté !", goToCart: "Voir le panier", continueShopping: "Continuer mes achats" },
} as const;
...
export function ProductRecommendations({ items, locale }: { items: ProductRecommendationDto[]; locale: Locale }) {
  ...
  const cartLabels = cartActionCopy[locale];
  return <section className="border-t border-border pt-7 lg:col-span-2" aria-labelledby="recommendations-title">
    ...
    <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">{items.map((item) => <article key={item.id} className="grid grid-cols-[92px_minmax(0,1fr)] gap-3 rounded-card border border-border bg-white p-3 sm:grid-cols-1">
      ...
      <div className="min-w-0"><h3 className="line-clamp-2 font-heading text-heading-sm font-bold text-text [hyphens:none] [overflow-wrap:normal] [word-break:normal]">{item.name}</h3>{item.variant ? <p className="mt-1 font-semibold text-text">{formatPrice(item.variant.priceCents, locale)}</p> : null}<div className="mt-3 grid gap-2">{addedId === item.id ? <ProductQuickViewAddedActions locale={locale} productName={item.name} quantity={1} labels={cartLabels} onContinue={() => setAddedId(null)} primaryActionRef={cartActionRef} /> : <><button type="button" disabled={!item.variant} onClick={() => { if (!item.variant) return; addCartItem({ variantId: item.variant.id, productId: item.id, slug: item.slug, name: item.name, variantLabel: item.variant.label ?? `${item.variant.weightGrams} g`, weightGrams: item.variant.weightGrams, priceCents: item.variant.priceCents, imageUrl: item.image?.url ?? null, locale }, 1); setAddedId(item.id); }} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-button bg-accent px-3 text-body-sm font-bold text-contrast disabled:opacity-50"><Plus className="h-4 w-4" />{labels.add}</button><Link href={productPath(locale, item.slug)} className="text-center text-body-sm font-semibold text-accent-hover underline underline-offset-4">{labels.view}</Link></>}</div></div>
    </article>)}</div>
  </section>;
}
