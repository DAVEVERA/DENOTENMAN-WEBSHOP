...
const MOCK_RECORD_PATTERNS = [
  /(^|[\s._@+-])(demo|dummy|lorem|mock|sample|seed|test|voorbeeld)([\s._@+-]|$)/i,
  /codex/i,
  /order\s*draft/i,
  /orderdraft/i,
];

function includesMockRecordText(value: string | null | undefined) {
  if (!value) return false;
  return MOCK_RECORD_PATTERNS.some((pattern) => pattern.test(value));
}

function isMockAdminOrder(order: AdminOrderListItem) {
  return [
    order.orderNumber,
    order.customerEmail,
    order.customerName,
    order.providerPaymentId,
  ].some(includesMockRecordText);
}

function isMockAdminPayment(payment: {
  orderNumber: string;
  customer: string;
  providerPaymentId: string | null;
}) {
...
export async function listAdminOrders(limit = 100) {
  ...
  return (data ?? []).map(mapOrder).filter((order) => !isMockAdminOrder(order));
}
...
export async function getAdminOrder(id: string) {
  ...
  return isMockAdminOrder(order) ? null : order;
}
...
export async function listAdminPayments(limit = 100) {
  ...
  return (data ?? [])
    ...
    .filter((payment) => !isMockAdminPayment(payment));
}
