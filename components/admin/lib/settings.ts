...
import { listAdminRoles, listAdminUserProfiles } from "./admin-users";
...
export async function getAdminSettingsStatus() {
  const [session, stored, users, roles] = await Promise.all([
    getAdminSession(),
    listStoredSettings(),
  ...
  return {
    ...
    users: users.map((user) => ({
      ...
      role: user.roleName,
      status:
        session?.email?.toLowerCase() === user.username.toLowerCase()
          ? "Actieve sessie"
          : user.active
            ? "Actief"
            : "Uitgeschakeld",
