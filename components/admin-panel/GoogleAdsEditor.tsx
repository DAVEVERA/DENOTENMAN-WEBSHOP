...
type Config = { status: string; headlines: string[]; descriptions: string[]; finalUrl: string; dailyBudgetMicros: number; keywords: string[]; published: boolean; lastError: string | null };
...
export function GoogleAdsEditor({ productId, initial, configured }: { productId: string; initial: Config; configured: boolean }) {
  ...
  async function request(body: object) {
    ...
    setMessage(response.ok ? "Google Ads-instellingen zijn verwerkt." : data?.error === "GOOGLE_ADS_CONFIG_MISSING" ? "De Google Ads-credentials ontbreken nog; het concept is wel veilig opgeslagen." : data?.error === "NO_KEYWORDS" ? "Voeg eerst zoekwoorden toe voordat je deze advertentie publiceert." : "De Google Ads-actie is niet gelukt.");
    if (response.ok) router.refresh();
  }
  function setKeywords(value: string) { setConfig({ ...config, keywords: value.split("\n").map((line) => line.trim()).filter(Boolean) }); }
  async function save(event: React.FormEvent) { event.preventDefault(); await request({ action: "save", headlines: config.headlines, descriptions: config.descriptions, finalUrl: config.finalUrl, dailyBudgetMicros: config.dailyBudgetMicros, keywords: config.keywords }); }
