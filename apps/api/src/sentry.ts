...
// ---------------------------------------------------------------------------
// hashUserId — sha256(id + pepper), truncated to 16 hex chars per ADR 0011.
// Pepper from SENTRY_USER_ID_PEPPER; falls back to plain sha256 if absent.
// ---------------------------------------------------------------------------
export function hashUserId(id: string): string {
  const pepper = process.env.SENTRY_USER_ID_PEPPER ?? "";
  const hash = crypto
    .createHash("sha256")
    .update(id + pepper)
    .digest("hex");
