...
import type { DesignAssetDto, PhotoRoomJobInput } from "@/lib/design-studio/photoroom-schema";
...
async function consumeDailyAttempt(jobId: string): Promise<number> {
  const dayKey = amsterdamDayKey();
  return prisma.$transaction(async (tx) => {
    await tx.designProviderUsage.upsert({
      where: { dayKey_provider: { dayKey, provider: "PHOTOROOM" } },
      create: { dayKey, provider: "PHOTOROOM", attempts: 0 },
      update: {},
    });
    const incremented = await tx.designProviderUsage.updateMany({
      where: { dayKey, provider: "PHOTOROOM", attempts: { lt: DAILY_LIMIT } },
      data: { attempts: { increment: 1 } },
    });
    if (incremented.count !== 1) throw new DesignStudioError("DAILY_LIMIT", "De dagelijkse PhotoRoom-limiet van 25 bewerkingen is bereikt.", 429);
    const usage = await tx.designProviderUsage.findUniqueOrThrow({ where: { dayKey_provider: { dayKey, provider: "PHOTOROOM" } } });
...
export async function createPhotoRoomDraft(input: {
  ...
  if (existing) {
    ...
    if (existing.status === "SUCCEEDED" && asset) {
      return { jobId: existing.id, asset: assetDto(asset), attemptsUsed: 0, dailyLimit: DAILY_LIMIT, replayed: true };
    }
  ...
  try {
    const attemptsUsed = await consumeDailyAttempt(job.id);
    const sourceBytes = await loadProductImageBytes(source.storageKey);
    const output = await runPhotoRoomEdit(sourceBytes, options);
    storageKey = `products/${safeId(source.productId)}/studio/photoroom/${safeId(job.id)}-${randomUUID()}.webp`;
    ...
    return { jobId: job.id, asset: assetDto(asset), attemptsUsed, dailyLimit: DAILY_LIMIT, replayed: false };
  } catch (error) {
...
export async function publishDesignAsset(input: { adminUserId: string; jobId: string; assetId: string }) {
  ...
  if (current.status === "PUBLISHED" && current.productImage) return { asset: assetDto(current), image: current.productImage, replayed: true };
  ...
  return { asset: assetDto(result.asset), image: result.image, replayed: false };
}

export function mapDesignStudioError(error: unknown): { status: number; body: { error: string; message: string } } {
  if (error instanceof DesignStudioError || error instanceof PhotoRoomError) {
