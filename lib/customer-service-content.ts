...
export function getCustomerServiceCopy(locale: Locale): CustomerServiceCopy {
  ...
  if (locale === "en") {
    return {
      ...
      entries: [
        ...
        { id: "shipping", category: "Delivery", question: "Where do you deliver and what does it cost?", answer: `Delivery is available in the Netherlands and Belgium. Netherlands: ${euro(locale, SHIPPING_POLICIES.NL.rateTiers[0].rateCents)}, free from ${euro(locale, SHIPPING_POLICIES.NL.freeShippingThresholdCents)}. Belgium: ${euro(locale, SHIPPING_POLICIES.BE.rateTiers[0].rateCents)}, free from ${euro(locale, SHIPPING_POLICIES.BE.freeShippingThresholdCents)}. Processing and delivery usually take about ${shippingMinimum}–${shippingMaximum} business days.`, links: [{ label: "Shipping and returns policy", href: shippingHref }] },
  ...
  if (locale === "fr") {
    return {
      ...
      entries: [
        ...
        { id: "shipping", category: "Livraison", question: "Où livrez-vous et quel est le tarif ?", answer: `La livraison est disponible aux Pays-Bas et en Belgique. Pays-Bas : ${euro(locale, SHIPPING_POLICIES.NL.rateTiers[0].rateCents)}, gratuite dès ${euro(locale, SHIPPING_POLICIES.NL.freeShippingThresholdCents)}. Belgique : ${euro(locale, SHIPPING_POLICIES.BE.rateTiers[0].rateCents)}, gratuite dès ${euro(locale, SHIPPING_POLICIES.BE.freeShippingThresholdCents)}. Le traitement et la livraison prennent généralement environ ${shippingMinimum}–${shippingMaximum} jours ouvrés.`, links: [{ label: "Politique de livraison et de retour", href: shippingHref }] },
  ...
  return {
    ...
    entries: [
      ...
      { id: "shipping", category: "Bezorgen", question: "Waar bezorgen jullie en wat kost het?", answer: `Bezorging is beschikbaar in Nederland en België. Nederland: ${euro(locale, SHIPPING_POLICIES.NL.rateTiers[0].rateCents)}, gratis vanaf ${euro(locale, SHIPPING_POLICIES.NL.freeShippingThresholdCents)}. België: ${euro(locale, SHIPPING_POLICIES.BE.rateTiers[0].rateCents)}, gratis vanaf ${euro(locale, SHIPPING_POLICIES.BE.freeShippingThresholdCents)}. Verwerking en bezorging duren gewoonlijk samen ongeveer ${shippingMinimum}–${shippingMaximum} werkdagen.`, links: [{ label: "Bekijk het verzend- en retourbeleid", href: shippingHref }] },
