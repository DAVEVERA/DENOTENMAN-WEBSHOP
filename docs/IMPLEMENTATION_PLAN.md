...
3. Fill config/next.config.ts: enable image remotePatterns for the CDN host.
...
1. lib/routes.ts: BASE_URL, contentPages list, and typed URL builders
   (home, collection, product, page, article) that all include the locale.
2. Implement each page.tsx as a server component that reads params
   (locale + slug) and calls the data layer. Add generateStaticParams
   where the set is known (collections, locales).
...
1. lib/queries.ts: getProductBySlug(slug, locale), getCollection(slug, locale),
   getFilteredProducts(categorySlug, locale, filters[]). Return flat, locale-
   resolved objects with CDN image URLs.
2. Filtering uses ProductAttribute key/value with AND semantics.

## Phase 7 — SEO surface
1. app/sitemap.ts: static + per-locale entries for collections, products,
