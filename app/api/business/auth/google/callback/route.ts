...
function loginPageError(error: string) {
  const url = new URL("/nl/zakelijk/inloggen", BASE_URL);
...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !expectedState || state !== expectedState) {
    return loginPageError("google_failed");
  }

  let email: string;
  try {
    const verified = await verifyGoogleBusinessAuthCode(code);
    if (!verified.emailVerified) return loginPageError("google_email_unverified");
  ...
  if (!account) return loginPageError("google_no_account");
  ...
  const response = NextResponse.redirect(new URL("/nl/zakelijk", BASE_URL));
