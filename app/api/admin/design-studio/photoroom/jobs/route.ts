...
import { photoRoomJobSchema } from "@/lib/design-studio/photoroom-schema";
import { createPhotoRoomDraft, mapDesignStudioError } from "@/lib/design-studio/service";
...
export async function POST(request: NextRequest) {
  ...
  const parsed = photoRoomJobSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
  ...
  try {
    const result = await createPhotoRoomDraft({ adminUserId: admin.id, idempotencyKey, options: parsed.data });
