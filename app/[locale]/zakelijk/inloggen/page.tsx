...
import { isGoogleBusinessLoginConfigured } from "@/lib/business-google-auth";
import { Logo } from "@/components/ui/Logo";
import { BusinessInvitationLoginForm } from "./BusinessInvitationLoginForm";
import { BusinessLoginOptions } from "./BusinessLoginOptions";

export const metadata: Metadata = { title: "Zakelijk inloggen | De Notenman", robots: { index: false, follow: false }, referrer: "no-referrer" };

export default async function ZakelijkInloggenPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ token?: string; error?: string }> }) {
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,#fff9da,transparent_38%),linear-gradient(180deg,#fff,#f7f2e6)] px-4 py-8 sm:py-14">
      <div className="mx-auto max-w-lg">
        ...
        <main id="main-content" className="mt-8 rounded-panel border border-border bg-surface p-5 shadow-card sm:p-8">
          ...
          {token ? (
            <BusinessInvitationLoginForm token={token} locale={locale} />
          ) : (
            <BusinessLoginOptions locale={locale} googleConfigured={isGoogleBusinessLoginConfigured()} oauthError={error} />
          )}
        </main>
      </div>
    </div>
