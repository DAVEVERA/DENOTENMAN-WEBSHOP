...
import { AFTERSALES_TOKENS } from "@/lib/aftersales/schema";
...
function previewDocument(content: AftersalesContent[Locale], trigger: AftersalesTriggerValue): string {
  ...
  return `<!doctype html><html lang="nl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(subject)}</title></head><body style="margin:0;background:#f6f3ee;font-family:Arial,sans-serif"><div style="padding:18px 10px"><div style="max-width:600px;margin:auto;overflow:hidden;border:1px solid #ded7ca;border-radius:12px;background:#fff"><div style="height:5px;background:#e0b200"></div><div style="padding:28px"><p style="margin:0;color:#806600;font-size:12px;font-weight:700;letter-spacing:.08em">DE NOTENMAN</p><h1 style="margin:12px 0 10px;color:#141414;font-size:24px;line-height:1.25">${escapeHtml(heading)}</h1><p style="color:#4f4a42;font-size:16px;line-height:1.65">${escapeHtml(body).replaceAll("\n", "<br>")}</p><div style="margin:18px 0;padding:14px;border:1px solid #ded7ca;border-radius:8px;background:#f6f3ee;font-size:14px"><strong>Bestelnummer:</strong> DN-2026-1842</div>${detail}<a href="#" style="display:block;margin-top:26px;min-height:44px;box-sizing:border-box;padding:14px;border-radius:8px;background:#e0b200;color:#141414;font-size:16px;font-weight:700;text-align:center;text-decoration:none">${escapeHtml(button)}</a></div><div style="padding:18px 28px;border-top:1px solid #ded7ca;color:#6e675c;font-size:13px">Vragen? Beantwoord deze e-mail; we helpen je graag.</div></div></div></body></html>`;
}
...
export function AftersalesFlowEditor({ initialFlow, initialDeliveries, provider }: Props) {
  ...
  const preview = useMemo(
    () => selectedStep ? previewDocument(selectedStep.content[locale], selectedStep.trigger) : "",
    [selectedStep, locale]
  );
  ...
  function updateContent(field: keyof AftersalesContent[Locale], value: string) {
    ...
    updateStep(selectedStep.id, (step) => ({
      ...step,
      content: { ...step.content, [locale]: { ...step.content[locale], [field]: value } },
    }));
  }
            ...
            <div className="min-w-0 p-4 sm:p-6">
              ...
              <div className="mt-5 grid grid-cols-1 gap-5">
                ...
                <div>
                  ...
                  <input id="mail-subject" value={selectedStep.content[locale].subject} maxLength={180} onChange={(event) => updateContent("subject", event.target.value)} className="mt-1 min-h-11 w-full rounded-button border border-border bg-background px-3 py-2" />
                </div>
                <div>
                  <label htmlFor="mail-preview" className="font-heading text-body-sm font-semibold">Previewtekst</label>
                  <input id="mail-preview" value={selectedStep.content[locale].previewText} maxLength={255} onChange={(event) => updateContent("previewText", event.target.value)} className="mt-1 min-h-11 w-full rounded-button border border-border bg-background px-3 py-2" />
                </div>
                <div>
                  <label htmlFor="mail-heading" className="font-heading text-body-sm font-semibold">Persoonlijke kop</label>
                  <input id="mail-heading" value={selectedStep.content[locale].heading} maxLength={180} onChange={(event) => updateContent("heading", event.target.value)} className="mt-1 min-h-11 w-full rounded-button border border-border bg-background px-3 py-2" />
                </div>
                <div>
                  <label htmlFor="mail-body" className="font-heading text-body-sm font-semibold">Bericht</label>
                  <textarea id="mail-body" rows={6} value={selectedStep.content[locale].body} maxLength={4000} onChange={(event) => updateContent("body", event.target.value)} className="mt-1 w-full rounded-button border border-border bg-background px-3 py-2 leading-6" />
                </div>
                <div>
                  <label htmlFor="mail-button" className="font-heading text-body-sm font-semibold">Knoptekst</label>
                  <input id="mail-button" value={selectedStep.content[locale].buttonLabel} maxLength={80} onChange={(event) => updateContent("buttonLabel", event.target.value)} className="mt-1 min-h-11 w-full rounded-button border border-border bg-background px-3 py-2" />
                </div>
              </div>
        ...
        <section className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          <div className="rounded-panel border border-border bg-surface p-4 shadow-card sm:p-6">
            ...
            <div className="mt-4 rounded-button border border-border bg-background p-3">
              ...
              <p className="mt-1 font-semibold text-text">{sampleValue(selectedStep.content[locale].subject)}</p>
            </div>
