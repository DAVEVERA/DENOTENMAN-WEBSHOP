    ...
    <fieldset className="grid gap-4 sm:grid-cols-2">
      ...
      <label className={labelClass}>
        ...
        <input
          ...
          onChange={(event) => onChange({ ...values, houseNumber: event.target.value })}
          className={inputClass}
        />
      </label>
      <label className={labelClass}>
...
function addressUnchanged(a: AddressFields, b: AddressFields): boolean {
  return a.street === b.street && a.houseNumber === b.houseNumber && a.postalCode === b.postalCode && a.city === b.city && a.country === b.country;
}
...
export function BusinessAccountEditForm({
  ...
  const [shippingEnabled, setShippingEnabled] = useState(currentShippingEnabled);
  const [shippingAddress, setShippingAddress] = useState(currentShippingAddress);
  const [shippingState, setShippingState] = useState<SaveState>("idle");
  const [shippingError, setShippingError] = useState<string | null>(null);
  const shippingUnchanged =
    shippingEnabled === currentShippingEnabled && addressUnchanged(shippingAddress, currentShippingAddress);
  ...
  const billingUnchanged = addressUnchanged(billingAddress, currentBillingAddress);
  ...
  async function handleProfileSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await patchBusinessAccount(businessAccountId, {
        ...
        customerNumber: customerNumber.trim().length > 0 ? customerNumber.trim() : null,
      });
      setProfileState("saved");
      router.refresh();
    } catch (error) {
      setProfileState("error");
      setProfileError(error instanceof Error ? error.message : "Onbekende fout");
    }
  }
  ...
  async function handleShippingSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await patchBusinessAccount(businessAccountId, {
        shippingEnabled,
        shippingStreet: shippingAddress.street.trim().length > 0 ? shippingAddress.street.trim() : null,
        shippingHouseNumber: shippingAddress.houseNumber.trim().length > 0 ? shippingAddress.houseNumber.trim() : null,
        shippingPostalCode: shippingAddress.postalCode.trim().length > 0 ? shippingAddress.postalCode.trim() : null,
        shippingCity: shippingAddress.city.trim().length > 0 ? shippingAddress.city.trim() : null,
        shippingCountry: shippingAddress.country.trim().length > 0 ? shippingAddress.country.trim() : null,
      });
      setShippingState("saved");
      router.refresh();
    } catch (error) {
      setShippingState("error");
      setShippingError(error instanceof Error ? error.message : "Onbekende fout");
    }
  }
  ...
  async function handleBillingSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await patchBusinessAccount(businessAccountId, {
        billingStreet: billingAddress.street.trim().length > 0 ? billingAddress.street.trim() : null,
        billingHouseNumber: billingAddress.houseNumber.trim().length > 0 ? billingAddress.houseNumber.trim() : null,
        billingPostalCode: billingAddress.postalCode.trim().length > 0 ? billingAddress.postalCode.trim() : null,
        billingCity: billingAddress.city.trim().length > 0 ? billingAddress.city.trim() : null,
        billingCountry: billingAddress.country.trim().length > 0 ? billingAddress.country.trim() : null,
      });
      setBillingState("saved");
      router.refresh();
    } catch (error) {
      setBillingState("error");
      setBillingError(error instanceof Error ? error.message : "Onbekende fout");
    }
  }
  ...
  async function handleDelete() {
    if (!window.confirm(`Weet je zeker dat je ${companyName} wilt verwijderen? Eerdere bestellingen en facturen blijven bewaard.`)) return;

    setDeleteState("saving");
    setDeleteError(null);

    try {
      const response = await fetch(`/api/admin/business-accounts/${businessAccountId}`, { method: "DELETE" });
      if (!response.ok) {
        const data = (await response.json().catch(() => null)) as { error?: string } | null;
        throw new Error(errorMessageFor(data?.error));
      }
  ...
  async function handleRestore() {
    ...
    try {
      const response = await fetch(`/api/admin/business-accounts/${businessAccountId}/restore`, { method: "POST" });
      if (!response.ok) {
        const data = (await response.json().catch(() => null)) as { error?: string } | null;
        throw new Error(errorMessageFor(data?.error));
      }
  ...
  return (
    <div className="space-y-8">
      {isDeleted ? (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-red-300 bg-red-50 p-4">
          <p className="text-body-sm font-semibold text-red-700">Dit account is verwijderd en verborgen uit de lijst.</p>
          <button
            type="button"
            onClick={handleRestore}
            disabled={deleteState === "saving"}
            className="inline-flex items-center justify-center rounded-button border border-red-400 bg-white px-4 py-2 font-heading text-body-sm font-semibold text-red-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {deleteState === "saving" ? "Bezig…" : "Herstellen"}
          </button>
        </div>
      ) : null}
      {deleteState === "error" && deleteError ? <p className="text-body-sm text-red-600">{deleteError}</p> : null}
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-accent-ink">Klantprofiel</p>
        <h2 className="mt-1 font-heading text-heading-sm text-text">Bedrijfsprofiel &amp; klantnummer</h2>
        <form onSubmit={handleProfileSubmit} className="mt-3 space-y-4 rounded-card bg-background p-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <label className={labelClass}>
              Bedrijfsnaam
              <input
                value={companyName}
                onChange={(event) => { setCompanyName(event.target.value); setProfileState("idle"); }}
                className={inputClass}
              />
            </label>
            <label className={labelClass}>
              ...
              <input
                value={contactName}
                onChange={(event) => { setContactName(event.target.value); setProfileState("idle"); }}
                className={inputClass}
              />
            </label>
            <label className={labelClass}>
              ...
              <input
                type="email"
                value={email}
                onChange={(event) => { setEmail(event.target.value); setProfileState("idle"); }}
                className={inputClass}
              />
            </label>
            <label className={labelClass}>
              ...
              <input
                value={phone}
                onChange={(event) => { setPhone(event.target.value); setProfileState("idle"); }}
                className={inputClass}
              />
            </label>
            <label className={labelClass}>
              ...
              <input
                ...
                onChange={(event) => { setCustomerNumber(event.target.value); setProfileState("idle"); }}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {profileState === "saved" && <span className="text-body-sm text-green-700">Opgeslagen</span>}
            {profileState === "error" && <span className="text-body-sm text-red-600">{profileError}</span>}
          </div>
        </form>
      </div>
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-accent-ink">Levering</p>
        <div className="mt-1 flex flex-wrap items-center justify-between gap-3">
          <h2 className="font-heading text-heading-sm text-text">Verzendadres</h2>
          <button
            type="button"
            onClick={() => { setShippingEnabled((prev) => !prev); setShippingState("idle"); }}
            aria-pressed={shippingEnabled}
            className={cn(
              "inline-flex items-center gap-2 rounded-button border px-4 py-2 font-heading text-body-sm font-semibold transition-colors duration-hover-fast",
              shippingEnabled
        ...
        <p className="mt-1 text-xs text-muted">
          Staat dit uit, dan haalt de klant zijn bestelling op bij De Notenman op de markt. Zet dit pas aan zodra
          verzenden voor deze klant mogelijk is.
        </p>
        <form onSubmit={handleShippingSubmit} className="mt-3 space-y-4 rounded-card bg-background p-4">
          <AddressFieldset legend="Verzendadres" values={shippingAddress} onChange={setShippingAddress} />
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="submit"
            ...
            {shippingState === "saved" && <span className="text-body-sm text-green-700">Opgeslagen</span>}
            {shippingState === "error" && <span className="text-body-sm text-red-600">{shippingError}</span>}
          </div>
        </form>
      </div>
      ...
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-accent-ink">Facturatie</p>
        <h2 className="mt-1 font-heading text-heading-sm text-text">Factuuradres</h2>
        <form onSubmit={handleBillingSubmit} className="mt-3 space-y-4 rounded-card bg-background p-4">
          <AddressFieldset legend="Factuuradres" values={billingAddress} onChange={setBillingAddress} />
          <div className="flex flex-wrap items-center gap-3">
            <button
              type="submit"
            ...
            {billingState === "saved" && <span className="text-body-sm text-green-700">Opgeslagen</span>}
            {billingState === "error" && <span className="text-body-sm text-red-600">{billingError}</span>}
          </div>
        </form>
      </div>
      ...
      {!isDeleted ? (
        <div className="rounded-card border border-red-200 bg-red-50/50 p-4">
          <h2 className="font-heading text-heading-sm text-red-800">Verwijderen</h2>
          <p className="mt-1 text-body-sm text-red-700">
            Verbergt dit account uit de lijst. Eerdere bestellingen en facturen blijven bewaard.
          </p>
          <button
            type="button"
            onClick={handleDelete}
            disabled={deleteState === "saving"}
            className="mt-3 inline-flex items-center justify-center rounded-button border border-red-400 bg-white px-4 py-2 font-heading text-body-sm font-semibold text-red-700 transition-colors duration-hover-fast hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
