...
export default async function NieuweBestellijstPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <p className="mt-2 text-body-sm text-muted">Kies een variant uit het assortiment of voeg een eigen regel toe. Namen en prijzen worden als momentopname bewaard; de klant kan de lijst alleen bekijken en afrekenen.</p>
      {account.status !== "APPROVED" ? <p role="alert" className="mt-5 rounded-card border border-amber-300 bg-amber-50 p-4 text-body-sm font-semibold text-amber-900">Keur dit account eerst goed.</p> : <BusinessOrderListForm accountId={account.id} options={options} mode="create" />}
    </div>
  );
}
